...
struct NewsRequest: BaseRequest {
    ...
    var path: String {
        return "users/CoderYuChong/received_events"
    }
...
struct NewsModel: DecodableJSON {
    let data: [NewsModelElement]
}

struct NewsModelElement: DecodableJSON, Codable {
    let id, type: String
    let actor: Actor
    let repo: Repo
    let payload: Payload
    let newsModelPublic: Bool
    let createdAt: String
    let org: Actor?
    
    enum CodingKeys: String, CodingKey {
        case id, type, actor, repo, payload
        case newsModelPublic = "public"
